...
multiple functions are needed */
  ...
  function totalAllTasks() {
    return {
      type: "html-keyboard-response",
      stimulus: function () {
        const task1 = jsPsych.data.get().filter({ task: "standard_trial" });
        const task2 = jsPsych.data.get().filter({ task: "simple_trial" });
        const task3 = jsPsych.data.get().filter({ task: "transparent_trial" });
